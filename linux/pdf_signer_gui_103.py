...
LIB_BEID = "/usr/lib/x86_64-linux-gnu/pkcs11/beidpkcs11.so"
CONFIG_FILE = os.path.expanduser("~/.pdf_signer_config.json")

class PDFSignerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Signateur et Trieur PDF eID - v1.0.2")
        try: self.root.attributes('-zoomed', True)
        except tk.TclError: self.root.attributes('-fullscreen', True)
        ...
        self.canvas.bind("<Button-4>", lambda e: self.on_mouse_wheel(-1, e.state))
        self.canvas.bind("<Button-5>", lambda e: self.on_mouse_wheel(1, e.state))

        # --- PANNEAU ACTIONS (BAS) ---
        # 1. On crée d'abord le conteneur des boutons
        self.bottom_frame = tk.Frame(root, pady=10)
        
        # 2. On crée la barre de statut
        self.status_bar = tk.Label(root, text="Prêt", bd=1, relief=tk.SUNKEN, anchor=tk.W, font=('Arial', 10), bg="#f0f0f0", pady=5, padx=10)
        
        # 3. FIX ERGONOMIE : On pack la barre de statut EN PREMIER pour qu'elle prenne le fond absolu [I]
        self.status_bar.pack(side=tk.BOTTOM, fill=tk.X)
        
        # 4. On pack le panneau des boutons EN DEUXIÈME (il se placera juste au-dessus) [I]
        self.bottom_frame.pack(side=tk.BOTTOM, fill=tk.X)

        # 5. On place les boutons à l'intérieur de leur panneau [I]
    ...
    def open_settings(self):
        ...
        # Validation propre alignée à droite (marge 20px) [I]
    ...
    def on_mouse_wheel(self, direction, state):
        if state & 4: self.adjust_zoom(0.2 if direction == -1 else -0.2); return
    ...
    def render_pdf_preview(self, pdf_path):
        try:
            ...
            self.lbl_info_nav.config(text=f"Page {self.current_page_num + 1}/{total_pages}  |  Zoom : {zoom_pct}%", fg="#FFFFFF" if self.dark_mode else "black")
    ...
    def sign_current_pdf(self):
        ...
        # On désactive les deux boutons pour éviter le double-clic
        self.btn_sign.config(state=tk.DISABLED, text="Connexion à l'eID...") # FIX : C'est le bouton vert qui change de texte !
        self.btn_reject.config(state=tk.DISABLED) # Le bouton rouge est juste désactivé sans changer de nom
    ...
    def _execute_signature_thread(self):
        try:
            ...
            cmd = ["pyhanko", "sign", "addsig", "--no-strict-syntax", "--field", f"1/257,734,337,814/Sig_{base.replace(' ', '_')}", "beid", "--lib", LIB_BEID, i_path, o_path]
            ...
            self.root.after(0, lambda: messagebox.showerror("Erreur", "La signature a échoué. Vérifiez votre carte et votre PIN."))
            self.root.after(0, lambda: self.show_status_message("Échec de la signature", bg_color="#F44336", fg_color="white"))
        finally: 
            # On remet le texte d'origine sur le bouton vert
